#include <franka_example_controllers_cyh/model_example_controller_cyh.h>
...
namespace franka_example_controllers_cyh{
    bool ModelExampleController_cyh::init(hardware_interface::RobotHW* robot_hw, ros::NodeHandle& node_handle)
    {
        franka_state_interface_ = robot_hw->get<franka_hw::FrankaStateInterface>();
        if(franka_state_interface_ == nullptr)
        {
            ROS_ERROR("ModelExampleController_cyh: Could not get Franka state interface from hardware");
            return false;
        }
        std::string arm_id;
        if(!node_handle.getParam("arm_id", arm_id))
        {
            ROS_ERROR("ModelExampleController_cyh: Could not read parameter arm_id");
            return false;
        }
        model_interface_ = robot_hw->get<franka_hw::FrankaModelInterface>();
        if(model_interface_ == nullptr)
        {
            ROS_ERROR_STREAM("ModelExampleController_cyh: Error getting model interface from hardware");
            return false;
        }
        ...
        try{
            ...
            ROS_ERROR_STREAM(
                "ModelExampleController_cyh: Exception getting franka state handle: " << ex.what());
            return false;
        }
        ...
        try{
            ...
            ROS_ERROR_STREAM(
                "ModelExampleController_cyh: Exception getting model handle from interface: " << ex.what());
            return false;
        }
        return true;
    }

    void ModelExampleController_cyh::update(const ros::Time&, const ros::Duration&)
    {
        if(rate_trigger_())
        {
...
PLUGINLIB_EXPORT_CLASS(franka_example_controllers_cyh::ModelExampleController_cyh,

...
{ 
    ...
    // 初始化
    this->KM_VS = new Krawtchouk_Moments_VS(order_min, order_max, delta_epsilon, lambda_order, resolution_x, resolution_y);
}

void Ros_KM_VS::Callback(const ImageConstPtr& image_color_msg, const ImageConstPtr& image_depth_msg)
{
    if(this->start_VS)
    {      
        // 数据转换
        ...
        // 计算相机速度并保存数据
        this->KM_VS->set_image_depth_current(depth_new);
        this->KM_VS->set_image_gray_current(img_new);
        if(this->KM_VS->flag_first_)
        {
            // 准备
            ...
            this->KM_VS->init_VS(lambda, epsilon, img_old, depth_old, img_new, camera_intrinsic, pose_desired);
            this->KM_VS->flag_first_ = false;
        }

        Mat camera_velocity = this->KM_VS->get_camera_velocity();
        this->KM_VS->save_data(camera_pose);
        ...
        cout << "iteration_num = " << this->KM_VS->iteration_num_ << endl;
        cout << "error = " << ((double)*(this->KM_VS->data_dom.error_pixel_ave_.end<double>() - 1)) << endl;
 
         // 判断是否成功并做速度转换
        if(this->KM_VS->is_success() || this->KM_VS->iteration_num_ > 500)
        {

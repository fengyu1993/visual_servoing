...
%% 读取DVS_data
[~, ~, DVS_data] = xlsread(location_name+'DVS_data_3D.xlsx');
DVS_camera_velocity = get_camera_velocity_data(DVS_data, camera_velocity_name, camera_pose_name);
DVS_camera_pose = get_camera_pose_data(DVS_data, camera_pose_name, camera_desired_pose_name);
DVS_camera_desired_pose = get_camera_desired_pose_data(DVS_data, camera_desired_pose_name, error_feature_ave_name);
DVS_error_feature = get_camera_error_feature_data(DVS_data, error_feature_ave_name, error_pixel_ave_name);
%% 读取DCT_VS_data
[~, ~, DCT_VS_data] = xlsread(location_name+'DCT_VS_data_3D.xlsx');
DCT_VS_camera_velocity = get_camera_velocity_data(DCT_VS_data, camera_velocity_name, camera_pose_name);
DCT_VS_camera_pose = get_camera_pose_data(DCT_VS_data, camera_pose_name, camera_desired_pose_name);
DCT_VS_camera_desired_pose = get_camera_desired_pose_data(DCT_VS_data, camera_desired_pose_name, error_feature_ave_name);
DCT_VS_error_feature = get_camera_error_feature_data(DCT_VS_data, error_feature_ave_name, error_pixel_ave_name);
DCT_VS_error_pixel = get_camera_error_pixel_data(DCT_VS_data, error_pixel_ave_name, order_name);
DCT_VS_order = get_camera_order_data(DCT_VS_data, order_name, "end");
%% 读取TM_VS_data
[~, ~, TM_VS_data] = xlsread(location_name+'TM_VS_data_3D.xlsx');
...
%% 读取KM_VS_data
[~, ~, KM_VS_data] = xlsread(location_name+'KM_VS_data_3D.xlsx');
...
%% 读取HM_VS_data
[~, ~, HM_VS_data] = xlsread(location_name+'HM_VS_data_3D.xlsx');
...
save("TM_VS_experience_data_3D.mat", "TM_VS_camera_velocity", "TM_VS_camera_pose", "TM_VS_camera_desired_pose", "TM_VS_error_feature", "TM_VS_error_pixel", "TM_VS_order");
save("KM_VS_experience_data_3D.mat", "KM_VS_camera_velocity", "KM_VS_camera_pose", "KM_VS_camera_desired_pose", "KM_VS_error_feature", "KM_VS_error_pixel", "KM_VS_order", "KM_VS_pxy");
save("HM_VS_experience_data_3D.mat", "HM_VS_camera_velocity", "HM_VS_camera_pose", "HM_VS_camera_desired_pose", "HM_VS_error_feature", "HM_VS_error_pixel", "HM_VS_order", "HM_VS_abxy");
imwrite(image_rgb_desired, "image_rgb_desired_3D.png");
imwrite(image_rgb_init, "image_rgb_init_3D.png");
